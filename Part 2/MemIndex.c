...
LinkedList* MemIndex_Search(MemIndex *index, char *query[], int query_len) {
  ...
  HTIterator* hash_itr = HTIterator_Allocate(wp -> postings);
  Verify333(hash_itr != NULL);

  while (HTIterator_IsValid(hash_itr)) {
    HTKeyValue_t doc_kv;
    HTIterator_Get(hash_itr, &doc_kv);
    ...
    HTIterator_Next(hash_itr);
  }
  HTIterator_Free(hash_itr);

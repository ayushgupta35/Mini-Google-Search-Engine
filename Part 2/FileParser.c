...
static void InsertContent(HashTable *tab, char *content) {
    ...
    if (isalpha(*cur_ptr)) {
      ...
      *cur_ptr = '\0';
      if (cur_ptr != content && isalpha(*(cur_ptr - 1))) {
        DocPositionOffset_t offset = word_start - content;
  ...
  if (word_start < cur_ptr && isalpha(*(cur_ptr - 1))) {
    DocPositionOffset_t offset = word_start - content;
    AddWordPosition(tab, word_start, offset);
  }
}
...
static void AddWordPosition(HashTable *tab, char *word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    WordPositions *wp2 = (WordPositions *) malloc(sizeof(WordPositions));
    wp2 -> word = str;
    wp2 -> positions = LinkedList_Allocate();
    LinkedList_Append(wp2 -> positions, (void*)((intptr_t) pos));
    HTKeyValue_t new_kv;
    new_kv.key = hash_key;
    new_kv.value = wp2;

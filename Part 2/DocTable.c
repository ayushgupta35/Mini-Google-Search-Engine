...
void DocTable_Free(DocTable *table) {
  ...
  // STEP 1.
  // Free the hash tables and their contents.
...
DocID_t DocTable_Add(DocTable* table, char* doc_name) {
  ...
  doc_id = (uint64_t *) malloc(sizeof(uint64_t));

  doc_copy = (char *) malloc(strlen(doc_name) + 1);
  Verify333(doc_copy != NULL);
  strncpy(doc_copy, doc_name, strlen(doc_name) + 1);
  res = FNVHash64((unsigned char *) doc_copy, strlen(doc_name));
  int val = HashTable_Find(table->name_to_id, res, &old_kv);
  Verify333(val != -1);
  ...
  *doc_id = table->max_id;
  ...
  kv.key = *doc_id;
  kv.value = doc_copy;
  val = HashTable_Insert(table->id_to_name, kv, &old_kv);
  Verify333(val != -1);
  ...
  kv.key = res;
  kv.value = doc_id;
  val = HashTable_Insert(table->name_to_id, kv, &old_kv);
  Verify333(val != -1);
...
char* DocTable_GetDocName(DocTable *table, DocID_t doc_id) {
  ...
  int val = HashTable_Find(table->id_to_name, doc_id, &kv);
  Verify333(val != -1);

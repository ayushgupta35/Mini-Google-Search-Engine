...
bool IsPathSafe(const string &root_dir, const string &test_file) {
  ...
  char resolved_test[PATH_MAX];

  if ((realpath(root_dir.c_str(), resolved_root) == nullptr) ||
      (realpath(test_file.c_str(), resolved_test) == nullptr)) {
    return false;
  }

  size_t len = strlen(resolved_root);
  if ((strncmp(resolved_test, resolved_root, len) != 0) ||
      (resolved_test[len] != '/' && resolved_test[len] != '\0'))
    return false;

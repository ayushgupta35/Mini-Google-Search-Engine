...
FileIndexReader::FileIndexReader(const string &file_name,
  ...
  uint32_t net_magic;
  Verify333(fread(&net_magic, sizeof(net_magic), 1, file_) == 1);
  header_.magic_number = ntohl(net_magic);

  uint32_t net_checksum;
  Verify333(fread(&net_checksum, sizeof(net_checksum), 1, file_) == 1);
  header_.checksum = ntohl(net_checksum);

  uint32_t net_doctable;
  Verify333(fread(&net_doctable, sizeof(net_doctable), 1, file_) == 1);
  header_.doctable_bytes = ntohl(net_doctable);

  uint32_t net_index;
  Verify333(fread(&net_index, sizeof(net_index), 1, file_) == 1);
  header_.index_bytes = ntohl(net_index);
  ...
  if (validate) {
    ...
    while (left_to_read > 0) {
      ...
      size_t n_read = fread(buf, 1, kBufSize, file_);
      Verify333(n_read > 0);
      left_to_read -= n_read;
      for (size_t k = 0; k < n_read; ++k) {
        crc_obj.FoldByteIntoCRC(buf[k]);
      }
    }

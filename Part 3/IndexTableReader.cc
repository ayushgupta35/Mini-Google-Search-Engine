...
DocIDTableReader* IndexTableReader::LookupWord(const string &word) const {
  ...
  for (IndexFileOffset_t &offset : elements) {
    ...
    // Seek to the offset of this element.
    ...
    header.word_bytes = ntohs(header.word_bytes);
    header.postings_bytes = ntohl(header.postings_bytes);
    ...
    for (int i = 0; i < header.word_bytes; i++) {
      ...
      char next;
      Verify333(fread(&next, sizeof(char), 1, file_) == 1);
      ss << next;
    }

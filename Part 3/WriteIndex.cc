...
namespace hw3 {
  ...
  //
  // Writes a header (BucketListHeader),
  //    a list of bucket records (BucketRecord),
  ...
  int WriteIndex(MemIndex *mi, DocTable *dt, const char *file_name) {
    ...
    Verify333(mi != nullptr);
    int status = WriteMemIndex(f, mi, cur_pos);
    if (status == kFailedWrite) {
      unlink(file_name);
      fclose(f);
      return kFailedWrite;
    }
    ...
    if (WriteHeader(f, dt_bytes, status) == kFailedWrite) {
      unlink(file_name);
      fclose(f);
      return kFailedWrite;
    }
  ...
  static int WriteHeader(FILE *f, int doctable_bytes, int memidx_bytes) {
    ...
    while (bytes_remaining > 0) {
      ...
      if (BUFFER_LEN >= bytes_remaining) {
        curr = bytes_remaining;
      } else {
        curr = sizeof(buf);
      }
      int read = fread(buf, 1, curr, f);
    ...
    // Seek to start of file and write the header
  ...
  static int WriteHashTable(FILE *f, IndexFileOffset_t offset, HashTable *ht,
    ...
    for (int i = 0; i < ht->num_buckets; i++) {
      // STEP 4.
      Verify333(ht != nullptr);

      LinkedList* bucket = ht->buckets[i];
      Verify333(ht->buckets != nullptr);

      int32_t chain_length = LinkedList_NumElements(bucket);
      int res = WriteHTBucketRecord(f, record_pos, chain_length, bucket_pos);
      if (res == 0)
        return kFailedWrite;

      if (chain_length > 0) {
        int written = WriteHTBucket(f, bucket_pos, bucket, fn);
        if (written < 0) {
          return kFailedWrite;
        }
        bucket_pos += written;
  ...
  static int WriteHTBucketRecord(FILE *f,
    ...
    bucket_rec.chain_num_elements = num_elts;
    bucket_rec.position = bucket_offset;
    ...
    // Write the number of elements (chain length).
  ...
  static int WriteHTBucket(FILE *f, IndexFileOffset_t offset, LinkedList *li,
    ...
    for (int i = 0; i < num_elts; i++) {
      ...
      ElementPositionRecord pos;
      pos.position = element_pos;
      pos.ToDiskFormat();
      ...
      if (fwrite(&pos, sizeof(ElementPositionRecord), 1, f) != 1) {
        LLIterator_Free(it);
        return kFailedWrite;
      }
      ...
      // Write the ElementPositionRecord in network order

      int written = fn(f, element_pos, kv);
      if (written < 0) {
        LLIterator_Free(it);
        return kFailedWrite;
      }
  ...
  static int WriteDocIDToPositionListFn(FILE *f,
    ...
    // size.  We must use reinterpret_cast<> here,
    //    because we are casting to/from
    // unrelated pointer types; see Google's C++ style guide.
    LinkedList *positions =
      reinterpret_cast <LinkedList*> (kv->value);
    int num_positions = LinkedList_NumElements(positions);
    int32_t num_positions_no = htonl(num_positions);
  ...
  // Loop through the positions list, writing each position out.
    ...
    LLPayload_t curr_payload;

    for (int i = 0; i < num_positions; i++) {
      // STEP 13.
      // Get the next position from the list.

      LLIterator_Get(it, &curr_payload);

      int64_t curr = reinterpret_cast <int64_t> (curr_payload);

      position = DocIDElementPosition(static_cast <int32_t> (curr));
  ...
  static int WriteWordToPostingsFn(FILE *f,
    ...
    uint16_t wordlen = strlen(wp -> word);
    ...
    // place in the file.
    if (fwrite(wp -> word, wordlen, 1, f) != 1) {
      return kFailedWrite;
    }

    // STEP 19.
    // Calculate and return the total amount of data written.
    return wordlen + ht_bytes + sizeof(header);
  }
}  // namespace hw3
